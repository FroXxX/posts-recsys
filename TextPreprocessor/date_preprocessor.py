...
class DatePreprocessor:
    ...
    def _replace_matched_date_w_day(self, match):
        ...
        if not date:
            ...
            if not ordinal:
                return ""
            tmp = found_date.replace(ordinal.group(0), self.ordinal_to_number[ordinal.group(0)])
            date = dateparser.parse(tmp)
        if not date:
            return ""
        new_str_for_date = f" {self.n2w.number_to_words(self.n2w.ordinal(date.day))}" \
    ...
    def _replace_matched_date_wo_day(self, match):
        found_date = match.group(0)
        date = dateparser.parse(found_date)
        new_str_for_date = f" {date.strftime('%B').lower()}" \
                           f" {self.n2w.number_to_words(date.year)} "
        return new_str_for_date
    ...
    def find_and_normalise_dates(self, text):
        ...
        month_pattern = r"\b(?:(?:" + "|".join(self.months) + r")\b|(?:" + "|".join(self.months_brief) + r")\b\.?)"
        date_pattern_wo_day = r"""
            # Month-Year 
            (?:
                """+month_pattern+r"""(?:[, ]+\d{4})
            )
        ...
        """
        modified_text = re.sub(date_pattern_w_day, self._replace_matched_date_w_day, text, flags=re.VERBOSE)
        modified_text = re.sub(date_pattern_wo_day, self._replace_matched_date_wo_day, modified_text, flags=re.VERBOSE)
