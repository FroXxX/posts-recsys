...
async def lifespan(app: FastAPI):
    ...
    if "WEB_LOAD" in os.environ and os.environ["WEB_LOAD"] == "1":
        logger.info("found WEB_LOAD variable set to 1, data will be loaded from database.")
        app.data_posts, app.data_users = load_data(db_connection)
    else:
        logger.info("WEB_LOAD is not set, data will be loaded from csv files.")
        app.data_posts = pd.read_csv("data/processed/posts_processed.csv", sep=";").set_index("post_id")
        app.data_users = pd.read_csv("data/processed/users_processed.csv", sep=";").set_index("user_id")
