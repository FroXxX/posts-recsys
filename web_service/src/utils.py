    ...
    class CustomDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            output = self.tokenizer(
                output, return_tensors="pt", padding="max_length", truncation=True
            )
...
def get_features_from_text(data_posts):
    ...
    distances = pd.DataFrame(
        cluster_model.transform(pca_processed),
        columns=[f"DistanceTo{ith}thCluster" for ith in range(1, 25)],
    )
...
def load_data(addr):
    ...
    query = """
        SELECT
        user_id,
        STRING_AGG(DISTINCT cast(post_id as varchar(5)), ',') as viewed_posts
